...
let currentPosition = currentPositionMarker = customDebugging = debugId = map = interval =intervalCounter = updateMap = false,
...
function generate_map( myOptions, canvasId ) {
	...
	locaties.forEach( el => {
		...
		try {
			...
			( localStorage.visited === undefined || isNumber( localStorage.visited ) ) ? localStorage[ el[ 0 ] ] = false : null
...
function _check_locations( event ) {
	...
	locaties.forEach( el => {
		...
		var locatie = { coords:{ latitude: el[ 3 ], longitude: el[ 4 ] } }
        ...
        debug_message( `Speler is binnen een straal van ${ el[ 2 ] } meter van ${ el[ 0 ] }` )
